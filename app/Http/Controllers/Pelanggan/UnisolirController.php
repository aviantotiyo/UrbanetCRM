...
class UnisolirController extends Controller
{
    /**
     * Mengubah status pelanggan dari 'isolir' menjadi 'active' dan mencatat log.
     */
    public function __invoke(Request $request, string $id)
    {
        ...
        // Jika status bukan isolir, tidak perlu diubah
        if ($client->status !== 'isolir') {
            return redirect()
                ->back()
                ->with('warning', "Pelanggan {$client->nama} tidak dalam status isolir.");
        }
        ...
        DataClientLogs::create([
            ...
            'client_id' => $clientId,
            'status'    => sprintf(
                'User %s telah membuka isolir Client (%s)',
                $actorName,
                $clientName
            ),
        ]);
        ...
        return redirect()
            ...
            ->with('success', "Pelanggan {$client->nama} telah diaktifkan kembali.");
    }
}
